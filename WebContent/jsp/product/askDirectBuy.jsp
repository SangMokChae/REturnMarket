...
<body>
  <div class="screen">
    <div class="screenForm">
      <main class="main">
		...
		<div class="askDirect-container">
			<h2 class="askDirect-title">판매자에게 즉시구매를 요청하세요</h2>
			<form action="askDirectBuy.pd" method="post">
				<div class="askDirect__form">
					<div class="uploader-name rows">
						<span class="col-3">상품 등록 유저</span>
						<input type="text" class="col-9" value="${pdBean.user_id }" readonly />
					</div>
					<div class="user-name rows">
						<span class="col-3">요청 유저</span>
						<input type="text" class="col-9" value="${user_id }" readonly /><br />
					</div>
					<div class="askWord-box rows">
						<span class="col-3">요청 글</span>
						<textarea name="askWord" id="summernote" class="col-9" required></textarea>
					</div>
				</div>
				<div class="btn-box">
					<button type="submit" class="btn">요청</button>
					<a href="pdView.pd?pdNum=${pdBean.pdNum }" class="btn" >뒤로</a>
				</div>
			</form>
		</div>
      </main>
  ...
  <script>
	  $('#summernote').summernote({
		  placeholder: "상품 등록하신 분께  즉시구매 요청을 해보세요.",
	      tabsize: 2,
        width: 100%,
	      height: 150
	      }); 
